...
export default function MyBlogsSection({
    ...
    const selectedBlog = useMemo(
        () => myBlogs.find((blog) => blog.id === selectedBlogId) || null,
        [myBlogs, selectedBlogId],
    );
                ...
                <article className={`rounded-2xl border p-5 shadow-sm ${listCardClass}`}>
                    <div className="flex flex-wrap items-start justify-between gap-3 pb-3">
                        ...
                        <div className="flex flex-col items-end gap-2">
                            <span
                                className={`rounded-full border px-2.5 py-1 text-xs font-bold uppercase tracking-wide ${statusClass[String(selectedBlog.status || 'PENDING').toLowerCase()] || statusClass.pending}`}
                            >
